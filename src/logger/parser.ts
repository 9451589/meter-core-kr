...
import { KillState, StatusEffectTarget, type GameState, type GameTrackerOptions } from "./data";
import {
  EntityTracker,
  EntityType,
  type Entity,
    ...
    this.#logger
      ...
      .on("InstanceZoneEnteredNotify", (pkt) => {
        ...
        if (this.#data.statQueryFilter.zone.has(parsed.zoneId)) {
          (logger as LiveLogger).writeLogFile = true;
        } else {
          (logger as LiveLogger).writeLogFile = false;
        }
      })
